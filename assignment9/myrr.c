...
static void update_curr_myrr(struct rq *rq){
        ...
        if(myrr_entity->update_num > TIME_SLICE) {
...
static void enqueue_task_myrr(struct rq *rq, struct task_struct *p, int flags) {
        ...
        printk(KERN_INFO "***[MYRR] enqueue: success cpu=%d,nr_running=%d,p->state=%ld,p->pid=%d \n", cpu_of(rq), myrr_rq->nr_running, p->state, p->pid);
}
static void dequeue_task_myrr(struct rq *rq, struct task_struct *p, int flags)
{
        struct myrr_rq* myrr_rq = &rq->myrr;
        ...
        if(myrr_rq->nr_running > 0) {
                ...
                list_del_init(&myrr_entity->run_list);
                myrr_rq->nr_running--;
                printk(KERN_INFO "***[MYRR] dequeue: the dequeued task is curr, set TIF_NEED_RESCHED flag cpu=%d,p->state=%ld,p->pid=%d,curr->pid=%d \n", cpu_of(rq), p->state, p->pid, rq->curr->pid);
...
struct task_struct *pick_next_task_myrr(struct rq *rq, struct task_struct *prev)
{
        ...
        next_entity = list_entry(queue->next, struct sched_myrr_entity, run_list);
